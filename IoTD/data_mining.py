...
def dataCollection(url):
    ...
    # used to split the stream data into dictionary valeus for further processing
...
while(True):
    mess=subscribeStatus() #wait for the status of data collection

    if(mess=="usbdata"): # this is to send data to SC
        prepareForPublish("combination.csv","usbdata1")
        print("SC asked for data")
    elif(mess=="abort"): # if SC decides to abort the data
        df = pd.DataFrame() # reinitiate the dataframe
        print("SC says abort the data and collect new data")
        datThread(url1,url2,url3)
    else:
        #this is to send the data to EDC encrypted
        start3 = time.time()
        print("Received the Key")
        
        # key to decrypt data from SC
        symmetricKey_KIS = b'aQOQxINtlrXU_HkbJywoMxfiFMXC-OToihHK2ApIeCs=' 
        KIS = Fernet(symmetricKey_KIS)
        encrypted=mess.encode()
        mess=KIS.decrypt(encrypted) # symmetric key to communicte data to EDC
        
        publishResult(str(number_of_rows),"decrypt_time")
        prepareForPublish1("test1.csv","usbdata_EDC",mess)
        print("send to EDC")
        # value=False
        end3 = time.time()
        print(end3-start3)
